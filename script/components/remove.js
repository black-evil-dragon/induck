/* Modules */
const fs = require('fs');
const path = require('path');
const colors = require('colors');


/* Variables */
const scssPath = '../induck/src/styles/scss/'
const componentsPath = '../induck/src/components/'
let { pages, components } = JSON.parse(fs.readFileSync('./data.json'));


/* Main function */
function removeComponent(componentName) {
    let run = false
    let text = ''
    let tempFileSCSS = fs.readFileSync(path.join(scssPath, 'components', `_index.scss`)).toString()

    try {
        tempFileSCSS = tempFileSCSS.replace(`@import "./${componentName}";`, '')
        fs.writeFileSync(path.join(scssPath, 'components', `_index.scss`), tempFileSCSS)

        fs.unlinkSync(path.join(componentsPath, componentName, 'index.tsx'))
        fs.unlinkSync(path.join(scssPath, 'components', `_${componentName}.scss`))

        fs.rmdirSync(path.join(componentsPath, componentName))

        components = components.filter(item => item !== componentName)
        run = true
    } catch (err) {
        run = false
        text = 'Ошибка при удалении файла или директории'
        console.log(err)
    }


    if (run) fs.writeFileSync('data.json', JSON.stringify({ pages, components }));

    return {
        run,
        text,
        componentName,
        components,
        pages,
    }
}

module.exports = { removeComponent }